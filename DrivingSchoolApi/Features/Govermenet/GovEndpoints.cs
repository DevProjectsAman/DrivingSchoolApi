    ...
    public static class GovEndpoints
    {
        public static void MapGovEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            group.MapGet("/GetOneGov/{id}", async (int id, ISender mediator) =>
            {
                ...
                return result == null
                    ? Results.NotFound(new { Success = false, Message = " Gov not found" })
                    : Results.Ok(new { Success = true, Data = result });
            });


            // Create
            group.MapPost("/CreateGov", async (CreateGovCommand cmd, ISender mediator) =>
            {
            ...
            // Update
            group.MapPut("/UpdateGov/{id}", async (int id, UpdateGovCommand cmd, ISender mediator) =>
            {
                if (id != cmd.GovId)
